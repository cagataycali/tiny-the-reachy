...
| `POST` | `/api/doa` | JSON {enabled} | 🔒 | `{enabled: bool}` — turn toward speech when face tracking has no lock (default on; `K` in the cockpit). Personas may call it from 127.0.0.1 without a key. |
