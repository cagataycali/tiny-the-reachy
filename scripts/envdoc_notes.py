...
NOTES = {
    ...
    "VOICE_LANG": "ISO-639-1 hint for input transcription (`tr`, `en`); unset = auto-detect, which turns room noise into phantom foreign-language turns.",
    "VOICE_VAD_THRESHOLD": "OpenAI server-VAD speech threshold 0–1 (default 0.5 = OpenAI's). Raising it makes barge-in over TINY's own speech harder; the XMOS board's hardware AEC already removes the echo.",
    "VOICE_TURN_DETECTION": "`server_vad` (default) or `semantic_vad` — how OpenAI decides the user's turn ended; interrupt_response/create_response are always on.",
    "VOICE_VAD_EAGERNESS": "`semantic_vad` only: `low` | `medium` | `high` | `auto` — how quickly it takes the turn.",
    "VOICE_XMOS_PARAMS": "XVF3800 audio-board parameters the voice persona writes at every start (runtime-only on the chip). Default `PP_NLATTENONOFF=0` — the factory value mutes your voice while Reachy speaks, which makes barge-in impossible. Empty disables.",
    "REACHY_DAEMON_PYTHON": "Interpreter of Pollen's daemon venv (default /venvs/mini_daemon/bin/python) — used for the USB fallback when the daemon's audio-config endpoint refuses int parameters.",
    "VOICE_TRANSCRIBE_PROMPT": "Free-text hint for the input transcriber (e.g. `Turkish or English, talking to a robot named TINY`) — steers it without pinning one language.",
    ...
    "TINY_PHOTO_QUESTION": "What `take_photo` asks the voice model about the frame when the caller gives no question.",
