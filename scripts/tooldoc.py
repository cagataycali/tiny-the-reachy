...
def render_module(mod: str, m: dict) -> str:
    ...
    lines = ["---", f"title: {m['title']}", f"description: \"{m['blurb'].replace('\"', '')}\"", "for: tool authors · prompt writers · anyone checking what a call really does",
             "proof: code", "---", "", HEADER.format(mod=mod), f"# {m['title']}", "",
             '!!! abstract "In 10 seconds"',
             f"    - {n} tool{'s' if n != 1 else ''} in `{m['file']}`: {names}.",]
    if m["moddoc"]:
        first = m["moddoc"].strip().split("\n\n")[0].replace("\n", " ")
        lines.append(f"    - {first}")
    lines += ["    - Signatures and defaults are read from the AST at every docs build; the descriptions are the docstrings the model itself reads.", ""]
    lines += ["| tool | does |", "|---|---|"]
    for t in m["tools"]:
        s = _sections(t["doc"])
...
def render_index(inv: dict[str, dict]) -> str:
    ...
    lines = ["---", "title: Tools reference", f"description: \"Every @tool TINY can call, generated from the code — {total} tools in {len(inv)} modules.\"",
             "for: tool authors · prompt writers · anyone checking what a call really does", "proof: code",
             "---", "", "<!-- generated by scripts/tooldoc.py — edit the docstrings, not this page -->", "", "# Tools reference", "",
             '!!! abstract "In 10 seconds"',
             f"    - **{total} tools in {len(inv)} modules**, generated from the `@tool` functions in `tools/` at every docs build (`scripts/tooldoc.py`).",
    ...
    for mod, m in inv.items():
        names = " ".join(f"`{t['name']}`" for t in m["tools"])
        lines.append(f"| [{m['title']}]({mod}.md) | {names} | {m['blurb']} [reference]({mod}.md) |")  # row starts with a link: tests/test_landing_tools.py reads it
    lines += ["", "</div>", "", "## Who gets what", "",
