...
MODULES = {  # (title, blurb ≤ 8 words — the index row and the page description)
    "reachy_motion": ("Motion", "head, antennas, body yaw, wake/sleep — clamped"),
    "reachy_expression": ("Expression", "recorded moves, fired while talking"),
    "reachy_state": ("State", "live pose and motor modes"),
    "reachy_camera": ("Camera", "frames, look-at, ask vision"),
    "reachy_audio": ("Audio", "Piper speech, sounds, speaker volume"),
    "head_tracking": ("Head tracking", "the daemon's face tracker (≥ 1.10)"),
    "turn_to_sound": ("Turn to sound", "ReSpeaker direction of arrival, no face locked"),
    "vision": ("Vision", "a frame plus a question, any persona"),
    "voice_bridge": ("Voice bridge", "text personas → voice persona queue"),
    "dispatch": ("Dispatch", "hand a task to another persona"),
    "memory": ("Memory", "the shared SQLite brain"),
    "telegram": ("Telegram", "text and photos to the owner's chat"),
    "prompts": ("Prompts", "read and override persona prompts"),
    "manage_messages": ("Manage messages", "compact the running agent's history"),
    "manage_tools": ("Manage tools", "create and hot-load tools at runtime"),
    "tiny_mcp": ("Fleet (tiny.technology MCP)", "fleet tools, only with `TINY_MCP=1`"),
}
...
def render_index(inv: dict[str, dict]) -> str:
    """Budget: ≤ 200 prose words (tools/docs_budget.py)."""
    ...
    lines = ["---", "title: Tools reference", f"description: \"Every @tool TINY can call, generated from the code — {total} tools in {len(inv)} modules.\"",
             ...
             '!!! abstract "In 10 seconds"',
             f"    - **{total} tools in {len(inv)} modules**, generated from the `@tool` docstrings — fix the docstring, not the page.",
             "    - Who gets which: [Personas](../../showcase/personas.md). Fleet tools only with `TINY_MCP=1`: [Fleet](../../MCP.md).", "",
             '<div class="cards cards--3" markdown>', "", "| module | tools | what it covers |", "|---|---|---|"]
